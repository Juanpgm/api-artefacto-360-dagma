...
try {
    ...
    Write-Host "? Archivo JSON le?do correctamente" -ForegroundColor Green
} catch {
    Write-Host "? ERROR: El archivo no es un JSON v?lido" -ForegroundColor Red
...
foreach ($field in $requiredFields) {
    ...
    if ($null -eq $value -or $value -eq "") {
        ...
        if ($field.Expected -and $value -ne $field.Expected) {
            Write-Host "   ??  Campo '$fieldName' tiene valor inesperado: $value" -ForegroundColor Yellow
        } else {
...
if (-not $allValid) {
    ...
    Write-Host "?? Aseg?rate de descargar el archivo desde Firebase Console > Cuentas de servicio" -ForegroundColor Yellow
    exit 1
}

# Mostrar informaci?n del proyecto
Write-Host "`n?? Informaci?n del proyecto:" -ForegroundColor Cyan
...
# Verificar que private_key contiene saltos de l?nea
if ($jsonObject.private_key -notmatch "\\n") {
    Write-Host "   ??  WARNING: private_key no parece tener saltos de l?nea (\\n)" -ForegroundColor Yellow
}

# Comprimir JSON (una sola l?nea, sin espacios)
...
Write-Host "   Tama?o original: $($jsonContent.Length) caracteres" -ForegroundColor Gray
Write-Host "   Tama?o comprimido: $($compressedJson.Length) caracteres" -ForegroundColor Gray
...
try {
    ...
    Write-Host "`n??  No se pudo copiar al portapapeles" -ForegroundColor Yellow
}
...
Write-Host "`n?? Pr?ximos pasos en Railway:" -ForegroundColor Yellow
...
Write-Host "   ??  NO subas el archivo JSON al repositorio" -ForegroundColor Red
Write-Host "   ??  Agrega *.json a .gitignore" -ForegroundColor Red
Write-Host "   ??  Elimina $outputFile despu?s de configurar Railway" -ForegroundColor Red

Write-Host "`n?? Verificar configuraci?n:" -ForegroundColor Yellow
...
try:
    ...
    if pk.startswith('-----BEGIN PRIVATE KEY-----'):
        ...
        print('??  private_key puede no tener formato correcto')
    
    print('\n? El JSON est? listo para usarse en Railway')
    sys.exit(0)
