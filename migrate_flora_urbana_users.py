"""
Migrates the `users` Firestore collection by updating the `grupo` field from
any Flora urbana variant (e.g., "cuadrilla", "Flora Urbana") to the canonical
value "flora_urbana".

Usage:
    python migrate_flora_urbana_users.py             # dry-run
    python migrate_flora_urbana_users.py --dry-run   # explicit dry-run
    python migrate_flora_urbana_users.py --apply     # write to Firestore
"""
...
def needs_migration(doc: dict) -> bool:
    """True if this user doc needs its grupo field migrated.
...
def run(dry_run: bool) -> None:
    ...
    for snap in docs:
        ...
        print(f"  {'[DRY RUN] Would update' if dry_run else 'Updating'}: id={snap.id!r}  grupo: {data.get('grupo')!r} -> {CANONICAL_GRUPO!r}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Migrate grupo field in `users` collection to canonical flora_urbana."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
